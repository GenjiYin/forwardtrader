...
class DualMovingAverage(bt.Strategy):
    ...
    def next(self):
        if self.p.datafeed.history_phase:
            ...
            # print(current_time, self.data0._name, self.data0.close[0], self.data1._name, self.data1.close[0])
            print(self.data0.datetime.datetime(), self.data0._name, self.data0.close[0], self.data1.datetime.datetime(), self.data1.close[0])
            # print(self.data0.datetime.datetime(), self.data0._name, self.data0.close[0])
            return
...
data2 = store.get_daily_data(instrument='SHFE.au2512', lookback=40)

# 订阅合约日线数据
# data_2 = store.get_daily_data(instrument='SHFE.cu2512', lookback=True)
