...
class Donchain(bt.Strategy):
    ...
    def __init__(self):
        self.donchain = Donchain_index(self.data0, period=self.params.donchain_period)    # 先上分钟的唐奇安
        self.ma = bt.indicators.SMA(self.data0, period=self.params.trend_period)
        self.adx = bt.ind.ADX(self.data0, period=self.params.adx_period)

    
    def next(self):
        if self.p.datafeed.history_phase:
        # 跳过回放数据阶段, 以免使用历史数据下单(必须存在, 可以不需要更改)
            print(self.data.datetime.datetime(), self.data1.close[0])
            return
        ...
        # 记录多头最低价和空头最高价
        if long_pos > 0:
            self.LowerAfterEntry = self.data0.low[0] if self.LowerAfterEntry is None else max(self.LowerAfterEntry, self.data0.low[0])
        
        elif short_pos > 0:
            self.HigherAfterEntry = self.data0.high[0] if self.HigherAfterEntry is None else min(self.HigherAfterEntry, self.data0.high[0])
        else:
            self.LowerAfterEntry = None
            self.HigherAfterEntry = None

        if long_pos > 0:
            Myprice = self.LowerAfterEntry - self.data0.open[0] * self.p.TR / 100
            if self.data0.low[0] <= Myprice:
                self.broker.sell_close(self.data0._name, size=long_pos)
                print(current_datetime, "多头跟踪止损")
                return

        if short_pos < 0:
            Myprice2 = self.HigherAfterEntry + self.data0.open[0] * self.p.TR / 100
            if self.data0.high[0] >= Myprice2:
                self.broker.buy_close(self.data0._name, size=short_pos)
                print(current_datetime, "空头跟踪止损")
                return


        # 过滤掉震荡行情
        if self.adx[0] < self.p.adx_filter:
            return
        
        if (current_time.hour == 9 and current_datetime.minute < 20) or (current_time.hour == 21 and current_datetime.minute < 20):
            # 前二十分钟不作任何交易
            return
        ...
        if pos == 0:
            if self.data0.close[0] > self.donchain.hig[0] and self.data0.close[0] > self.ma.sma[0]:
                ...
                self.broker.buy_open(self.data0._name, size=1)
            
            elif self.data0.close[0] < self.donchain.low[0] and self.data0.close[0] < self.ma.sma[0]:
                print(current_datetime, "开空")
                self.broker.sell_open(self.data0._name, size=1)

# 创建引擎
cerebro = bt.Cerebro()
# 连接天勤（请在 MyStore 中配置您的登录信息）
store = MyStore(key='xxxx', value='xxxxxxxxxx', strategy_name='布林带')
