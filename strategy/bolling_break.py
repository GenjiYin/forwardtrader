...
class Donchain_index(bt.Indicator):
    ...
    def next(self):
        self.lines.hig[0] = max(self.data.high.get(0, self.params.period))
        self.lines.low[0] = min(self.data.low.get(0, self.params.period))
        self.lines.avg[0] = sum(self.data.close.get(0, self.params.period)) / self.params.period
...
class Donchain(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.adx = bt.ind.ADX(self.data1, period=self.params.adx_period)
